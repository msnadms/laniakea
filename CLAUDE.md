...
Seven rare finds: black holes, ruined Dyson spheres, Matrioshka brains, Nicoll-Dyson beams, Shkadov thrusters and the homeworlds of the civilisations that built them — or, for a few living ones, the Alderson disk they rebuilt their home into. `src/game/anomalies.ts → generateAnomalies(galaxy)` places them, and every rate lives in `constants.ts` (`ANOMALY_*`). They are derived, never stored on the galaxy: `gameStore.galaxyAnomalies` is recomputed wherever `makeGalaxy` runs, and an anomaly never changes its host's data — every difference is applied while rendering. The Milky Way hosts none.
