...
- **Emergency fuel** (`store/emergencyFuel.ts`, `store/emergencyRecovery.ts`) — the Peregrine carries a separately persisted sealed reserve of 30 Exotic Matter and 25 Helium-3. Navigation warns before operational fuel falls below that margin and asks before breaking the seal; construction cannot draw from it, and neither can an ordinary logistics dispatch — except that a route dispatch expected to deliver more exotic matter (or more helium-3) to the ship than that resource's own cost (`RoutePreview.selfFundingExotic`/`selfFundingHelium` in `logisticsStore.ts`) may borrow that resource's shortfall, since the dispatch repays it in the same transaction; the two fuels are judged independently, so a route can dip into one reserve while paying the other from operational fuel alone. Each fuel gauge includes the reserve in its displayed total, with an amber protected segment and vertical threshold fixed at 15% for legibility rather than scaled against capacity; operational fuel uses the remaining 85%. Clicking a gauge after the reserve is breached transfers available operational fuel back behind both reserve lines. A ship unable to afford its cheapest in-galaxy jump can request fuel from a supplied living colony (adding 0.5 probe attention) or recover to Sol with one jump's fuel, 25% raw-cargo loss, and 1 attention. Emergency attention is capped just below lethal lock so recovery itself cannot turn the softlock escape into an immediate death.
