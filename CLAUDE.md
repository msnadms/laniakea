...
`npm test` runs the vitest suite (galaxy generation, galaxy shapes, projection, anomalies). `ANOMALY_ODDS=1 npx vitest run src/game/anomalies.odds.test.ts --silent=false` prints civilisations per supercluster over 100 superclusters (target ~1.5), black holes over 3,000 galaxies, and each megastructure's rate per civilisation. `ANOMALY_CIVILIZATION_CHANCE` is 1 in 20,000 against ~31k galaxies per supercluster, so tests that need civilisations find them with `findCivilizationSeeds` (`civilizationSeeds.testutil.ts`) rather than by sampling.
...
Eight rare finds: black holes, ruined Dyson spheres, Matrioshka brains, Nicoll-Dyson beams, Shkadov thrusters and the homeworlds of the civilisations that built them — or, for a few living ones, the Alderson disk they rebuilt their home into, sometimes guarded by an Alcubierre cannon. `src/game/anomalies.ts → generateAnomalies(galaxy)` places them, and every rate lives in `constants.ts` (`ANOMALY_*`). They are derived, never stored on the galaxy: `gameStore.galaxyAnomalies` is recomputed wherever `makeGalaxy` runs, and an anomaly never changes its host's data — every difference is applied while rendering. The Milky Way hosts none.
...
- Homeworld — every civilisation has one, placed after the black holes with no RNG draw: the unoccupied G/K star outside the bulge and starbursts nearest the region centre — inside the region if one is free, otherwise anywhere — and only then the nearest unoccupied host of any class. Its kind is `Civilization.homeKind`: `ANOMALY_ALDERSON_CHANCE` of living civilisations roll `aldersonDisk` instead, drawn right after the living roll.
- Populated worlds — living civilisations only, placed last: 2–3 (`ANOMALY_POPULATED_*`) unoccupied F/G/K stars outside the bulge and starbursts inside the region. They are not anomalies: `GalaxyAnomalies.populated` holds their host ids, nothing catalogues them, and their only difference is one `populated` planet.
- Alcubierre cannon — Alderson civilisations only: `ANOMALY_CANNON_CHANCE` rolls `Civilization.cannon` as the last civilisation draw, and it is placed after the populated worlds with no RNG draw, on the unoccupied, unpopulated F/G/K star outside the bulge and starbursts farthest from the centre inside the region. `direction` points away from home, in the plane.
