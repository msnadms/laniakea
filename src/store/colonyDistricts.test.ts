...
describe('colony districts and jobs', () => {
  it('fills jobs in player priority order with both scarce and surplus population', () => {
    const c = colony({ population: 2, districts: { ...colony().districts, farm_district: 1, research_district: 1 } });
    expect(filledJobs(c)).toMatchObject({ farmer: 2, researcher: 0 });
    expect(filledJobs({ ...c, jobPriority: ['researcher', ...DEFAULT_JOB_PRIORITY.filter(job => job !== 'researcher')] })).toMatchObject({ farmer: 0, researcher: 2 });
  ...
  it('integrates output and clamps unattended catch-up', () => {
    const c = colony({ population: 100, districts: { ...colony().districts, research_district: 1 }, jobPriority: ['researcher', 'farmer', 'steward'] });
  ...
  it('degrades output in proportion to unmet upkeep without directly removing population', () => {
    const c = colony({ population: 100, districts: { ...colony().districts, research_district: 1 }, supplies: { nutrients: 100_000, 'helium-3': 4 }, jobPriority: ['researcher', 'farmer', 'steward'] });
  ...
  it('builds a district straight from the ship, charging its rares and a jump of fuel', () => {
    ...
    useStockpileStore.setState({ materials: {}, rares: { zero_point_capacitor: 1, frame_dragging_gyro: 1 } });
    ...
    expect(useStockpileStore.getState().rares).toMatchObject({ zero_point_capacitor: 0, frame_dragging_gyro: 0 });
  ...
  it('razes a district, salvaging one of the rares it was built from', () => {
    const c = colony({ population: 1200, districts: { civilian_district: 2, farm_district: 1, research_district: 0 } });
    useColonyStore.setState({ colonies: { [c.key]: c } });
  ...
  it('feeds a full civilian district from one farm, and stalls a district with no upkeep in store', () => {
    const fed = colony({ population: 1000, districts: { civilian_district: 1, farm_district: 1, research_district: 0 }, supplies: { nutrients: 10_000 } });
  ...
  it('stalls growth when civilian districts cannot cover the population', () => {
    ...
    expect(content.amenityRatio).toBeCloseTo(((civilian.baseAmenities ?? 0) + 200 * (civilian.output.amenities ?? 0))
      / (600 * AMENITIES_PER_PERSON + AMENITIES_PER_WORKING_DISTRICT));
    expect(content.population).toBeGreaterThan(staffed.population);

    const crowded = colony({ population: 1000, districts: { civilian_district: 1, farm_district: 2, research_district: 2 } });
  ...
  it('grows a freshly chartered colony on the amenities its own districts provide', () => {
    const chartered = colony({ population: 100, districts: { civilian_district: 1, farm_district: 1, research_district: 1 } });
  ...
  it('refuses to raze the last civilian district under an inhabited colony', () => {
    const c = colony({ population: 500, districts: { civilian_district: 1, farm_district: 1, research_district: 0 } });
    useColonyStore.setState({ colonies: { [c.key]: c } });
    useStockpileStore.setState({ materials: {}, rares: {} });
  ...
  it('migrates installed assemblies and labor from an old save', () => {
    ...
    expect(next.districts).toMatchObject({ civilian_district: 2, research_district: 0 });
