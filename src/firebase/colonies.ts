...
export async function saveColony(uid: string, colony: Colony): Promise<void> {
  const { installed: _installed, exportedLines: _exportedLines, fedMs: _fedMs, labor: _labor, populationTier: _populationTier, selfSufficientMs: _selfSufficientMs, lastShipmentAt: _lastShipmentAt, ammo: _ammo, ...current } = colony as Colony & Partial<LegacyColony> & { ammo?: number };
