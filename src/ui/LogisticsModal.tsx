...
function LogisticsModalInner({ onClose }: { onClose: () => void }) {
  ...
  const handleDispatch = useCallback((routeId: string) => {
    ...
    if (collectedKeys !== false) {
      ...
      for (const k of route.nodeKeys) {
        const ext = liveExtractors[k];
        const col = liveSettlements[k];
        let nodeId: string | null = null;
        if (ext) nodeId = getSystemKey(ext);
        else if (col) nodeId = `colony:${col.galaxySeed}|${col.systemId}`;
  ...
  const draftOrderChain = useMemo(() => {
    ...
    for (const k of draftNodeKeys) {
      ...
      if (ext) {
        ...
        if (!byNodeId.has(sk)) {
          const node: DraftNode = { nodeId: sk, name: getSystemName([ext]), keys: [], isColony: false };
        ...
        if (!byNodeId.has(colId)) {
          const node: DraftNode = { nodeId: colId, name: col.systemName || col.planetName, keys: [], isColony: true };
